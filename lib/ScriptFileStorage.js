...
var DEFAULT_EXTENSIONS = ['.js','.jsx','.es6'];
...
function createAllJSPattern(extensions) {
  return '{' + [].concat(extensions).map(toGlob).join(',') + '}';
}

function createConventionalDirsPattern(extensions) {
  var DIRS = ['lib','test'],
    MODULES = ['node_modules/**/*.js'];

  function dirGlobber(dir) {
    return [].concat(extensions).map(function(ext) {
      return dir + '/' + toGlob(ext);
    });
  }
  function toWildcard(ext) {
    return '*' + ext;
  }

  return '{' +
    [].concat(extensions).map(toWildcard)
      .concat(DIRS.map(dirGlobber))
      .concat(MODULES).join(',') +
    '}';
}
...
function ScriptFileStorage(config, session) {
  ...
  this._allJSPattern = createAllJSPattern(config.extensions || DEFAULT_EXTENSIONS);
  this._conventionalDirsPattern = createConventionalDirsPattern(config.extensions || DEFAULT_EXTENSIONS);
}
  ...
  async.waterfall(
    [
      ...
      function(dir, isRoot, cb) {
        var pattern = isRoot ? this._allJSPattern : this._conventionalDirsPattern;
  ...
  this._isApplicationRoot(
    ...
    function handleIsStartDirectoryApplicationRoot(result) {
      if (!result) {
        ...
        this.listScripts(startDirectory, this._allJSPattern, callback);
      }
    }.bind(this)
  );
};
