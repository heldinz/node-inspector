...
var definitions = {
  'help': {
    alias: 'h',
    description: 'Display information about available options.',
    usage: {
      '--help': '           display short list of available options',
  ...
  'preload': {
    ...
    usage: {
      '--preload': '',
      '--no-preload': '    disable preloading files'
    },
    _isNodeInspectorOption: true,
    default: true
  },
  'extensions': {
    alias: 'ext',
    type: 'array',
    description: 'Array of file extensions to load',
    usage: {
      '--extensions .js': 'load only *.js files'
    },
    _isNodeInspectorOption: true,
    default: ['.js','.jsx','.es6']
  },
...
function Config(argv, NODE_DEBUG_MODE) {
  ...
  if (!!parsedArgv.extensions) defaults.extensions = [];
...
function normalizeOptions(options) {
  ...
  checkExtensionsOption(normalizedOptions);
...
function checkExtensionsOption(options) {
  ...
  options.extensions = options.extensions.map(testExtension);
}
